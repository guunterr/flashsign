...
void run_flashsign3_cuda(int X, int Y, fp16 *Q, fp16 *K, fp16 *V, fp16 *O){
    ...
    constexpr uint BY = 256;
    constexpr uint BX = 16;
    ...
    for (int j = 0; j < 5; j++)
    {
        ...
        {
            summarised_times[j] += times[i * 8 + j] / ceil_div(Y, BY);
        }
    }
    printf("Times summarised\n");
    for (int j = 0; j < 8; j++)
    {
        printf("%d: %lld\n", j, summarised_times[j]);
    }
    printf("Kernel 3: QLoad = %lld, KVBufferWait = %lld, Dot = %lld, Norm = %lld, Write = %lld\n", summarised_times[0], summarised_times[1], summarised_times[2], summarised_times[3], summarised_times[4]);
