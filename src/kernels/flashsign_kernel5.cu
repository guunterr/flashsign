...
__global__ void kernel(int X, fp16* Q, fp16* K, fp16* V, fp16* O, long long int * global_timers) {
    ...
    for (int KV_Block = 0; KV_Block < X; KV_Block += BX)
    {
        ...
        for (int i = 0; i < BX; i++)
        {
            float s = __half2float(KVBuffer[active_buffer][0][laneIdx][warpIdx * 32 + i]);
            z += s * s;
        }
        __syncthreads();
    }
    float z_rsqrt = 1/(sqrtf(z) + EPSILON);
    // printf("Z[%d] = %f\n", blockIdx.x * BY + tix, z);
    if(isnan(z_rsqrt)){
       printf("Z[%d] = %f, Z_RSQRT[%d] = %f\n", blockIdx.x * BY + tix, z, blockIdx.x * BY + tix, z_rsqrt);
    }
    fp16 l = __float2half(z_rsqrt);
    __syncthreads();
    //Write out O
...
torch::Tensor flashsign_5(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    CHECK_INPUT(Q);
    CHECK_INPUT(K);
    CHECK_INPUT(V);
    //change
    ...
    flashsign_kernel5::run_kernel(X, Y, Qp, Kp, Vp, Op);
